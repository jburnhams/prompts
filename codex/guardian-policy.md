<!-- Source: openai/codex, codex-rs/core/assets/guardian/. Verbatim, three
     files concatenated. `policy_template.md` is the synchronous reviewer's
     own system prompt; `policy.md` is the default tenant policy that fills
     its `{{ tenant_policy_config }}` slot (and the guardian-v2 classifier's);
     `node_repl_policy.md` is the computer/browser-use addendum. -->

# `policy_template.md` — the reviewer prompt
...
# `policy.md` — the bundled default tenant policy
...
# `node_repl_policy.md` — computer/browser-use addendum

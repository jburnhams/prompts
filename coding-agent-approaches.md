...
| A specific fast-search-tool brand preference stated in the prompt | Codex explicitly recommends `rg`/`ripgrep` over `grep` for speed. (Correction: this row previously also credited OpenHands with the same preference — checked directly, OpenHands's prompt says "use efficient tools like find, grep, and git commands," no `rg` mention. See `agent-tool-surfaces.md` for the full per-source search-tool breakdown.) |

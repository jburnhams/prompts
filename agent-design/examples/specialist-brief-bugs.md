...
- The orchestrator wrote only `<focus>` — two sentences of direction
  ...
  (`review.md` §4 rule 1).
- **Absent on purpose** (§4 rule 2): the `<existing_comments>` block
  (dedup happens after candidates return, and Bob's thread must not
  prime or steer the finder), and any `<conventions>` tag (this repo
  has none; the conventions specialist wasn't even launched).

...
8. `orchestration.md` — the run ledger: the layer *above* a run, which
   ...
   as the accepted cost. The one thing still marked as needing its own
   pass is that reconciliation: a ticket reopened after `abandoned`, a
   reassignment mid-`handoff`, and a human closing a ticket as done
   while a task is still running are all unworked.

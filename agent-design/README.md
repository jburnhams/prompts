...
| Batch `Read` with per-file ranges | `files: [{path, start_line?, end_line?}]` — several files per call, each with its own optional inclusive line range | Single-path `Read` with `offset`/`limit` (the original draft, Claude Code's shape), relying on parallel tool calls for batching; a glob-driven bulk reader (Gemini CLI's `read_many_files`) | Turn count *is* this design's run budget (`formats.md` §7), so collapsing five reads into one call buys headroom under the same ceiling — a stronger argument here than in an interactive agent, where parallel tool calls already cover most of it. `start_line`/`end_line` over `offset`/`limit` because every line number the model has came from a `cat -n` read, a Grep hit, a diff hunk or a review finding, and ranges need no arithmetic to build from those; `offset`/`limit` is accepted and normalised. Cline's SDK is the precedent, including the failure mode its schema text warns about — models put the range in a separate array element instead of on the path's object — which is why that warning is carried verbatim into the description. The cost is a result format with per-file blocks and per-file errors, and an overall size budget across entries; a bad entry reports inline instead of failing the call |
| Tools over MCP on Google ADK | Keep every tool schema, description and result format exactly as specified, and absorb the substrate's effects at four named seams (result envelope, harness-record channel, per-call classification, mode enforcement) — one stdio server per run, file tools and `Bash` in the same server | Designing the tool layer natively for MCP semantics (results as `structuredContent`, permissions via annotations, elicitation for `AskUser`); treating the transport as fully invisible | The model's view genuinely is unchanged — names, schemas and result text all survive, and ADK doesn't prefix tool names by default. But the transport is not neutral in three places, all verified in ADK's source rather than assumed: it returns the entire `CallToolResult` to the model, so text is JSON-escaped and anything in `structuredContent` is duplicated into context (hence: one text block, harness record out of band); MCP annotations are per-tool, so the per-call predicates in the metadata table have to live in a `before_tool_callback` or the server; and `tool_filter` is advertising, not enforcement, so the design's structural mode-narrowing has to be a differently-started server process rather than a filtered view. The per-run server also makes the process boundary the state boundary for read-before-edit, which is why `Bash` must not be split into a separate server from the file tools |

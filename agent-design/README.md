...
- **Coding mode is one system prompt covering three `mode` values**,
  not three agents: `plan` (read-only — investigate, then either
  `AskUser` or produce a plan for a later run), `implement` (the
  default — edit and verify), and `investigate` (read-only, reporting
  without a forward-looking plan — this covers both open-ended
  investigation and being asked to assess existing code without
  changing it; the task instruction carries that distinction, not the
  mode). `plan` and `implement` are two separate *runs*, not two phases
  of one run — see `formats.md` §6 for exactly how a plan produced by
  one run reaches the next.
...
| Read-only mode enforcement | Structural: the harness doesn't wire `Edit`/`Write` at all in `plan`/`investigate` runs, for the orchestrator or its `general-purpose` delegates; prompt rules remain as a second layer | Prompt-only enforcement (the original draft: tools wired in every mode, "never call them" as instruction) | Same argument as the scratch-directory row: a structural boundary can't be forgotten. Gemini CLI strips agent-kind tools in code; Composio scopes per role at the tool level (`agent-subagent-architectures.md` §6); `agent-permissions-approval.md`'s core finding is that serious sources layer prompts *on top of* structural gates, never instead |
| `review_only` mode | Folded into `investigate` — no separate mode value; the task instruction carries "assess, don't just explore" | A distinct `review_only` mode, defined as "treat the same as `investigate` for write purposes" | The two modes had identical authorization and differed only in framing, and the name collided confusingly with the review *entrypoint*. A mode that's defined as "same as X" isn't pulling its weight — one fewer mode value to keep consistent across `system-prompts.md`/`tools.md`/`formats.md` |

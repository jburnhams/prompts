...
| One vocabulary for lens / role / kind | **Kinds** — one open, service-validated string set (`bugs`, `security`, `conventions`, `visual`, `ux`, …) resolving to **three payloads**: prompt scope, tool grants, and (reserved) context sections. `role` stays as the field name in the two schemas that ship it; "lens" stays as the prose word for a kind in its scoping use | Four parallel taxonomies drifting apart (what was happening); renaming the live `role` fields for tidiness; a closed enum maintained in two places | `context-files.md` had already declared the review lens *to be* a kind ("a review specialist asks for its own lens — the fan-out is already keyed on exactly that"), so this was one concept under three names rather than a merge. **Supersedes the `(subagent_type, role)` tool-scoping key** recorded a day earlier as the same mechanism stated as a special case: the general form is `(subagent_type, kinds)` → capability set. Two rules make the tool payload safe and are what distinguish this from a privilege-escalation surface — a kind **selects from** a harness-defined capability set and never defines one (the corpus tags sections; harness config owns kind→tools, so editing a conventions file can never grant a tool), and kinds are **declared at spawn by the spawner**, never by an agent about itself mid-run. Renaming live schema fields was rejected on this folder's own supersession discipline: churn without a decision behind it |
| The layer above a run | A **run ledger** (`orchestration.md`): a harness-side task record with dependencies, a lease, an attempt list and a give-up policy. v1 slice is one `Complete` field (`spun_off`) plus one table | Continuing to design task splitting, the PR steward and generalized suspension as three separate subsystems (what `future.md` was doing); a chat-agent-style claimable board with agent-facing claim/heartbeat/complete tools (OpenClaw's Workboard shape) | Three deferred items were each blocked on part of the same missing noun — a unit of work that outlives the process that discovered it. In an archetype-1 tool the human *is* that ledger; this design removed the human (`README.md` above) and nothing took over the job, which is why the gap is ours and not the field's. Agent-facing claim tools were rejected because our runs are **dispatched**, not self-claiming: the harness can take the lease before the run starts and settle it from the `Complete` report, which keeps the tool surface at its current size (the brief's leanness rule) and removes an injection surface — nothing a run reads can talk it into completing or reassigning a task record |
| Oversight of the ledger | **Non-model**: give-up, staleness and repeated-failure are queries over stored fields, evaluated by a deterministic dispatch pass | A periodic supervisor/overseer run that reads the board and decides what is stuck, retried or escalated | Four reasons, and the third is the one that would have been found late: the judgments are cheap and deterministic; a supervisor reasons only over what is rendered into its context, so a large ledger gets summarised and a stalled task disappears in the summary; task titles and attempt summaries are written by runs that read untrusted ticket and PR text, so a supervisor reading them lets untrusted content **choose the next run** — reach nothing else in this design grants; and the field agrees, OpenClaw shipping an `orchestratorProfile` field and an `autoDecompose` flag that only *mark* a card as a candidate, plus a supervision spec stating "There is no separate Supervisor plugin" |
| What a run may do about work outside its scope | **File it, don't do it**: a `Complete.report.spun_off` proposal with `relation: blocks_this \| follows_this`. `blocks_this` ends the run with `status: "blocked"` and the originating task waits in the ledger — no held transcript | A mid-run `ProposeTask` tool (the obvious shape); stretching the run to cover the extra paths; v1's current answer alone — record the out-of-scope write and let the review entrypoint catch it | A mid-run tool invites filing the spinoff *and carrying on anyway*, which is the behaviour the item exists to prevent; the report is where a decision to stop belongs. It also answers `future.md`'s open question "what does the originating run do while it waits" as **nothing — it ends**, which is strictly cheaper than `medium.md` §4c's suspended run and is why that section is now narrowed to the transcript-must-survive case. v1's post-run cross-check stays as the backstop for the run that wandered without noticing, which no spinoff mechanism can catch |
| The role invariant, stated | **A role may file, propose or report; it may not act on what it oversees.** The ledger's new actor (whatever files a spinoff) inherits it: a run may file a task, never dispatch one | Leaving it implicit, as it had been — the validator already can't post, a coding run can't commit, a `plan` run can't implement | It was three coincidences until the ledger added a fourth actor with a genuinely new power. Reading the same invariant across five independent roles in one external system (Custodian proposes config and a human applies; the exec reviewer can allow or escalate but structurally **cannot deny**; ClawSweeper "recommends with evidence, never executes the close"; the skill reviewer emits pending proposals; memory consolidation places supplied entries but "never author[s] replacement prose") is what promoted it from pattern to rule |
| Where the ledger lives | **Its own store**, holding `issue_key` as a reference, with the tracker receiving **projections** on four transitions (spinoff filed, blocked, abandoned, dependency cleared) through the existing `AddComment` path | Jira as the ledger with an agent-owned status field; a second store with no tracker projection at all | Only three fields in the task record — `state`, `depends_on`, `priority` — map onto tracker fields without abuse; a lease with a deadline, an attempt list with run ids, a frozen scope carrying an authority token and per-task budgets are agent-*execution* state, and custom-fielding them is how integrations rot. Jira-as-ledger would also have to open `WriteJira` (deferred in `medium.md` §5b) before the ledger did anything useful. `done` is deliberately not a projection — the Complete report already reaches the ticket, and a second comment on top is noise. **The accepted cost is reconciliation**: a human can move a ticket without telling the ledger, so the standing rule is the tracker wins on intent, the ledger wins on execution state, and conflicts project a comment rather than silently reconciling |
| Concurrency key | **(repository, branch)**, with `handoff` counting as occupied | (repository, ticket assignee) — the first draft; repository only; no concurrency rule in v1 | The assignee version justified itself as *reviewer attention*, which was a proxy for the resource rather than the resource. Keyed on branch, the argument is direct: a review of the handed-off PR can dispatch an `implement` run (`medium.md` §1b) against **that same branch**, so starting unrelated work there guarantees a conflict with the agent's own output. It has a substrate prerequisite that the deployment meets — **a worktree or clone per run** — which also makes parallel runs in one repository real; where that fails it degrades correctly to repository-only, since one checkout is one branch. Two things the design now has to state rather than assume: the harness owns checkout provisioning (the agent still never creates or switches branches), and a spun-off task's branch is decided at dispatch, not by the run |
| Downstream of `handoff` | An external committer commits and opens the PR; the task leaves `handoff` on a **PR merged/closed webhook**, reusing the PR-activity subscription `medium.md` §1 already needs | The committer reporting back directly; a human closing the task; `handoff` as terminal with `done` never reached | Confirmed as a deployment fact, which is what promotes `handoff` from a deferred state to a v1 one and gives `suggested_commit_message` a consumer. The webhook wins over a direct callback because it needs no second integration and sees human pushes to the branch for free; the committer's only obligation is writing the PR reference back once. Merged → `done`; closed unmerged → `abandoned` with the reference kept, because a closed PR is a decision, not a failure to retry |
| Spinoff gating (`filed → ready`) | **Asymmetric**: `blocks_this` auto-chains, `follows_this` is gated on an approving reply | Gated always; auto-chained always; surface-only (humans file every task) | Each half has its own reason rather than one rule applied twice. A `blocks_this` parent is *already stalled*, so gating makes two tasks wait on a decision nobody was asked to prioritise, and the work is by construction a prerequisite of an already-approved ticket. A `follows_this` is new scope the run invented — exactly where a human should look, and nothing is stalled while they do. The asymmetry is not a safety claim about blocking work: it is that a wrong auto-dispatch is bounded by the caps, while a wrong gate on a blocking task stalls a ticket indefinitely, and only one of those failures is self-limiting |
| Ledger caps | **2** spinoffs per run, spinoff **depth 0**, **8** tasks per originating ticket, **2** attempts per task | Moderate (3 / depth 1 / 15 / 3) and loose (5 / depth 2 / 30 / 3) variants | Depth 0 — a spun-off task may not itself spin off — is the load-bearing one, because it kills runaway fan-out by *shape* rather than by budget arithmetic, and shape is checkable at file time where a total-task budget is only violated after the fact. That matters most in a first version, when nobody has calibration for what normal looks like. The named cost: a genuinely layered dependency (fix the type, then callers, then tests) cannot decompose itself in one pass and instead reaches a human as a recorded observation. Depth 1 is the obvious first relaxation and needs only a constant |
| Ledger scope | **Coding runs only in v1.** Review findings that need code enter as coding tasks with `source: "review_finding"` | Both run types from the start; review runs plus a reviewer-filed task | A review run is dispatched by an event, posts everything it concludes, and terminates — it carries no obligation that outlives the run, which is the entire reason the ledger exists. Keeping reviewers out also avoids re-answering "may file, may not dispatch" for a role whose whole discipline is that it does not act. `medium.md` §3f's stateful re-review is the case that will eventually pull them in, and the table already has the shape |
| Ticket closed under a running task | **Let the run finish, then abandon** with `abandon_reason: "ticket_closed"`, posting the report anyway. Dependent spinoffs go to `blocked`, not abandoned; a task already in `handoff` is left to its PR's own events | Cancel the run immediately; ignore tracker state entirely | Never kill a run mid-flight: it may be seconds from a useful report, and cancelling discards a working tree and a transcript for nothing. Work that happened should not be invisible because the ticket closed while it happened. Dependents survive because a closed parent ticket says nothing about whether the spun-off work is still worth doing, and abandoning a chain on one ticket-hygiene click is the worse error |
| Run persistence and budget exhaustion | Both adopted: a **persistence rule** in the coding prompt (declare `blocked` only after the same obstacle stops you twice, with a different approach tried in between) and **`budget_exhausted` as its own `Complete` status**, distinct from `failed` | Persistence rule only; status only; neither, leaving `formats.md` §7 as-is | Adapted from OpenClaw's session goals, which pair "block only after the same blocker on 3 consecutive turns" with a token budget whose exhaustion is a terminal state separate from failure. The persistence rule's shape is what makes it adoptable — it is checkable from the transcript rather than from introspection, so a run cannot satisfy it by feeling stuck. The status earns a schema change because "ran out of room" and "could not do it" are different **retry** decisions, and with `max_attempts: 2` spending an attempt on the former is expensive; §7's repeated-failure query counts only the latter |
| Provenance claims in review findings | Three-valued — **introduced / made visible / pre-existing** — and no attribution to a person. `unknown` is deliberately absent | Porting OpenClaw's four-word vocabulary and its raw-parent verification protocol wholesale; leaving provenance unbounded as today | Review roles have **no `Bash`** (`review.md` §2), so a finder has no git history and cannot verify a provenance claim at all — which makes OpenClaw's protocol untransferable but its *distinction* valuable: `made visible` is a real reportable category that neither "introduced" nor "pre-existing" covers, and this design had no word for it. `unknown` is dropped because a finder that cannot establish provenance should not be making a provenance claim. Role separation (line author / PR author / merger / automation / fixer are five roles, none implying another) is kept in full, because these comments land on a PR a person reads. If a `history` lens is ever wired with `Bash`, adopt the protocol wholesale and `unknown` returns with it |
| `Read` continuation axes | Three: `start_line`, `end_line`, and **`char_offset`** — a 0-indexed character position for resuming a truncated over-long line | Two axes, with over-long lines simply clipped and unrecoverable (the previous spec) | The 2000-character per-line cap is a real dead end for a minified bundle, a long generated constant or a single-line JSON fixture — the model is told the line was truncated and has no way to see the rest. OpenClaw's within-line cursor is the cleanest answer in the collection and costs one optional integer. It also fits the existing rule that every cap names its exact next call |
| Envelope delimiters | **Both** tags carry the run nonce — attribute on the open, tag name on the close | An asymmetric scheme with the nonce on the close only, which one sentence of `context-files.md` §4 incorrectly claimed was the design | Corrected rather than decided: the template, the `<existing_comments>` precedent it cited, and the shipped reviewer-core prompt all already said both tags; only the prose said otherwise. The reason symmetry is load-bearing here is specific to a **multi-tier** envelope — the attributes carry privilege (`binding="policy"` means non-overridable), so a body that could mint a plausible higher-binding *open* tag would not need to close anything. Single-envelope schemes can afford the asymmetry; this one cannot |
| Invisible characters in the conventions corpus | **v1**: a resolution-time **refusal**, not a transformation — a section carrying `Cf`/`Cc`/`U+2028`/`U+2029` outside a fenced code block fails resolution, and §1b's existing rule means the run does not start | HTML-escaping the body (OpenClaw's approach for its untrusted blocks); running `formats.md` §1's sanitiser over it; leaving it as a named residual (my first call, overturned) | Escaping buys nothing once both delimiters carry a nonce, and costs something real — a conventions file is full of angle brackets in generics/JSX/HTML examples, and it is a document the model is meant to quote from, not a payload to be treated as data. Note where OpenClaw points its own escaper: results, attachments and operator text, never documents to be followed. The residual that remains is a *human*-facing one — a bidi or zero-width payload is approved by a reviewer seeing one thing while the model receives another, which attacks the exact assumption ("a human committed this") that justifies following the file at all. A resolution-time refusal keeps the never-transform rule and the byte-identical-to-disk guarantee intact, because nothing is rewritten — the only outcome is that a run does not begin. It also fails toward the *right* human: the attack works by making a reviewer see something other than what the model gets, so the mitigation has to end at a reviewer. Fenced code blocks are exempt, since a conventions file may legitimately contain examples about Unicode handling, and a rule that cannot express that is a rule teams disable. Accepted cost, stated rather than discovered: a stray soft hyphen from a word processor stops every run against that repository until someone removes it — loud, self-naming, one character to fix |
| Persistence vs. the final-turn nudge | **`budget_exhausted` outranks `blocked`** on a nudged turn; the obstacle goes in the report body, and an obstacle named in a previous attempt's report counts as already-seen | Letting `blocked` win when a real obstacle exists; making the nudge text persistence-aware so the model picks | A collision introduced by adopting both rules in one pass. Resolving it toward the budget status turned out to *improve* the persistence rule rather than merely settle a conflict: the ledger retries with a raised budget, the retry's envelope carries the previous report verbatim, and the same obstacle there is the second occurrence — so the rule spans **attempts** rather than turns. That is where it belonged, because a second look at an obstacle is only worth having if it gets a real context window, and the within-a-run version guaranteed the opposite. Bounded by `max_attempts: 2`: a task that exhausts context twice is abandoned, not raised a third time |
| Where suppressed depth-0 spinoffs live | **Nowhere new** — `outcome.report` already stores the completing run's report verbatim, and `spun_off` is part of it; the dispatcher simply declines to act on the entries | A `suppressed_spinoffs[]` field plus a fifth projection transition; a field with no projection | The machinery was already there, and the human-facing half too: a `blocks_this`-shaped overflow lands the task in `blocked`, which §10 projects, and a `follows_this`-shaped one completes so the report reaches the ticket by the ordinary route. Adding a field and a transition would have duplicated a stored report to solve a problem that only looked open because §11 and §2 were written in different passes |
| Reconciliation model | **Invalidation, not sync.** The two stores hold disjoint field sets — the tracker owns intent, the ledger owns execution — so there is no symmetric conflict, only a tracker change that invalidates an execution assumption | Bidirectional sync with conflict resolution; a last-writer-wins field merge; leaving it undefined | Reframing it this way is what keeps a small problem from becoming a distributed-systems one: you do not merge two versions, you notice a premise changed and either re-derive or stop. It is the same shape as the context service's cache design, where every tier reduces to `(source repo, ref)` so entries invalidate themselves — and it should be, because the ledger's copy of a ticket **is** a cache |
| Reconciliation verbs | **Adopt, stop, say** — and never *fix* | A `WriteJira`-backed ledger that corrects the tracker; silent auto-correction on the ledger side | Falls out of a permissions decision made for unrelated reasons: `AddComment` is the only outward channel and `WriteJira` is deferred (`medium.md` §5b), so there are only three things the ledger can do. With three verbs there is no reconciliation logic to get subtly wrong, and every branch's failure mode is "a human is told," which is the failure mode this design prefers everywhere. It also means the tracker's `done` and the ledger's `done` never have to be reconciled into one claim — a decision that comes due the moment `WriteJira` lands |
| When reconciliation runs | **At transitions**: a ticket re-read before dispatch and at settlement, plus an opportunistic sweep for non-running tasks. No webhook required | A tracker-event subscription as the mechanism; a periodic reconciler process | §8's argument in a different place — a watcher must be running, can silently stop, and reasons over what it happens to be shown, where a check at a transition is a query that either ran or did not. A tracker-event subscription is a latency optimisation on top: without it reconciliation is *late*, not absent, which is the right degradation |
| Reopening an abandoned ticket | **A new task**, with `supersedes` pointing at the old one | Resurrecting the abandoned task | Resurrection forces a bad choice: reset the attempt budget and lose the give-up signal, or keep it and re-abandon immediately. The old task's `abandon_reason` is also a fact about what happened that should not be edited away — the same supersession discipline this folder applies to its own decisions, for the same reason |
| Reassignment mid-`handoff` | **A no-op** | Moving the task between owner buckets; blocking on it | Not a decision so much as a case that stopped existing. Under the earlier `(repository, ticket assignee)` key this was a real problem with no clean answer; keying on `(repository, branch)` deleted it, since the ledger holds no assignee-derived state and projections reach the ticket the new assignee is now reading. Recorded because a key change removing a whole row from the reconciliation table is the usual sign the key was named after the wrong thing first time |
| Long outward payloads | `AddComment` takes `body` **or** `body_ref`, a ref through `artifacts.md`'s existing closed scheme set; above a threshold the harness attaches and says so ([`generative.md`](./generative.md) §2a) | Inline-only (the original v1 schema); a general `Publish` tool; extending the ref channel to `Complete.report` | `artifacts.md`'s "bytes never enter context" was stated only for reads. `ArtifactData`'s `data`-or-`file_path` pair is the worked precedent, described in its own schema as existing "so a large document need not pass through the conversation" (`../agent-generative-output.md` §4d). `Complete.report` is excluded because it is a bounded structured object, not a blob — a ref there would be a second way to say one thing. **Supersedes** `tools.md`'s `AddComment.body`-always-required row |
| Degradation reporting | A closed `refusal` code alongside the existing prose on every harness-side refusal or fallback; no `ok` code ([`generative.md`](./generative.md) §2b) | Prose only (the original v1 behaviour); an open string field; a per-tool ad-hoc shape | The prose steers the model and stays. The code is what `eval.md` counts and `memory.md` keys on, and neither can branch on a sentence. `artifact-components`' publish verifier is the precedent — a security policy stated as a catalogue a model can program against (`script-not-blessed`, `unknown-data-island`, `island-sentinel-ambiguity`, `banner-state-mismatch`). An open string field becomes a second, worse prose channel |
| The `implement` completion gate's content | The fixed checklist is prefixed by a **computed findings block** with FAIL/WARN severities and a fixed line naming what it did *not* check ([`generative.md`](./generative.md) §2c) | Keeping the checklist entirely prompted; adding an LLM judge to the gate; computing everything and dropping the prompted half | `dataviz`'s `validate_palette.py` is the model: compute what is arithmetic, separate compensable from non-compensable, document the coverage gap, and name the calibration that would silently invalidate it. The gate's existing argument (SWE-agent's `review_on_submit_m`, `../agent-self-verification.md` §2) is that a mechanical gate cannot be talked out of firing — an LLM judge would forfeit exactly that. The gate's trigger, reset rule and status semantics are unchanged |
| `AskUser` shape | Options become `{id, label, implication?}`; `<resumed_answer>` gains `option_id` and a three-value `match` enum (`exact`/`inferred`/`none`) with the reply text still carried verbatim; plan-mode questions may carry `draft_ref` ([`generative.md`](./generative.md) §2d) | Plain-string options (the original v1 schema); free-text parsing on resume; a full `workshop`-style republish loop | The `workshop` skill's structural claim is that the question and the answer share a schema, so the resume path never interprets prose. `match="none"` is the valuable value — it detects the non-answer `formats.md` §5 already anticipates but leaves Forge to spot by reading. The republish loop needs a page runtime Forge has no use for yet; `generative.md` §4 holds it behind a named trigger. **Supersedes** `formats.md` §5's bare-text `<resumed_answer>` |
| Skills, if ever added | Fifth rule for `context-files.md` §12: a skill's instructions are read, its code is **executed and never `Read` into context** — and that executable is a trust decision under §12's existing provenance rules | Treating bundled code as ordinary readable context | `webapp-testing` states it directly ("black-box scripts rather than ingested into your context window"); the `pptx` skill's ~1.1 MB of ISO-IEC 29500-4 XSD and ~875-line validators is the evidence that a skill's size is only irrelevant to its context cost if this rule holds |
| Artefact delivery channel | Refs and schemas; **never** an in-band delimiter grammar inside the assistant message ([`generative.md`](./generative.md) §6) | `:::artifact{…}`-style directives (LibreChat, Claude.ai's original Artifacts prompt); `<boltArtifact>` tags | Charges the payload to context twice (authored, then carried in history); the delimiter grammar is load-bearing enough to need a per-model-family error-recovery block in the prompt — LibreChat ships *"Common mistakes to avoid: Don't split the opening `:::` line…"* to its OpenAI-family models and not its Anthropic ones; and the failure mode is a half-rendered artefact rather than an error |
...
8. `orchestration.md` — the run ledger: the layer *above* a run, which
   the design was missing. A durable task record with dependencies,
   leases, attempts and a give-up policy, so work can outlive the
   process that discovered it. Specifies task splitting
   (`Complete.report.spun_off`, with `blocks_this`/`follows_this` as the
   only two relations), a deterministic dispatcher with a one-task-per-
   owner rule, and health checks as queries over stored fields rather
   than as a supervisor run — §8 argues that position and states the
   role invariant the ledger's new actor has to satisfy (**a role may
   file, propose or report; it may not act on what it oversees**).
   Supersedes two `future.md` items and narrows `medium.md` §4c. §10
   settles where the ledger lives — its own store, with the tracker
   receiving projections on four transitions — and names reconciliation
   as the accepted cost — and **§10a** then works it out: reconciliation
   is invalidation rather than sync (the two stores hold disjoint field
   sets, so there is no symmetric conflict to merge), it runs at
   transitions rather than through a watcher, and the ledger's inability
   to write to the tracker leaves it exactly three verbs — adopt, stop,
   say. It can never fix.
9. `medium.md` — the medium-term roadmap: concrete post-v1 upgrades
   with design sketches, plus the named escalation triggers.
10. `future.md` — long-horizon and measurement-gated work, tracked
   separately so it isn't silently re-litigated later.
11. `adk.md` — implementation-substrate notes, not design: how this
...
12. `eval.md` — the measurement harness: the hostile-workspace fixtures,
...
13. `generative.md` — what this design takes from
   [`../agent-generative-output.md`](../agent-generative-output.md), the
   research pass on agents that make an artefact rather than a code
   change. Almost none of it is about pictures: the transferable
   material is about the boundary between what the model authors and
   what the harness renders, which Forge crosses in four places it was
   improvising. Five v1 adoptions, none of them a new tool — a write-side
   ref channel on `AddComment`, a closed `refusal` vocabulary on every
   degradation path, a computed half for the `implement` completion
   gate, `AskUser` options with stable ids and a three-valued resume
   match, and the execute-don't-read rule for skills — plus the
   structured-representation rule stated once in `formats.md` §8
   instead of four times, one roadmap promotion (a published review
   briefing, with its trigger named), three `future.md` items, and four

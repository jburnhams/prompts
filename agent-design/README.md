...
| Where the ledger lives | **Open** — recorded as a hypothesis, not a decision (`orchestration.md` §10): execution state in its own store holding `issue_key` as a reference, with the tracker receiving projections through the existing `AddComment` path | Jira as the ledger with an agent-owned status field; a second store with no tracker projection at all | Genuinely undecided, and this folder's discipline is to say so rather than write a decision after the fact. The pull toward Jira is real (one place for humans, already in the envelope, already durable); the pull away is that leases, attempt lists, frozen scope and per-task budgets are agent-execution state, and tracker writes are a permission surface v1 does not have — `WriteJira` is deferred in `medium.md` §5b. Needs its own pass, including what happens when a human closes the ticket underneath a running task |
...
8. `orchestration.md` — the run ledger: the layer *above* a run, which
   ...
   Supersedes two `future.md` items and narrows `medium.md` §4c. §10 is
   an open question: whether the ledger is its own store or Jira with an
   agent-owned status field.

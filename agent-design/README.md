...
| Read-before-edit and unchanged-file dedup | Both harness-side state: `Edit`/`Write` fail on an unread path; a re-read of an unchanged file returns a stub pointing at the earlier result | Prompt-only instruction for read-before-edit (the original draft); no dedup at all | Read-before-edit is the same structural-gates argument as the git-write blocklist, and it's enforced in code in the one product that documents it. Dedup is new to this design and is the largest token saving available for its build cost: on an implement run that re-reads the same handful of files, an unchanged re-read drops from thousands of tokens to about thirty. No source other than Claude Code was found doing it, which makes it the one place this design is ahead of its own references rather than behind them |
...
fetch paths and commits recorded in [`sources.md`](../sources.md)). It
produced no change to which tools Forge has, and a substantial change to
what each one is required to do around the call: `tools.md` gained an
implementation contract (three channels per tool, output-format policy,
self-describing caps, spill-to-scratch, errors-as-instructions, tolerant
parsing, the shared `cat -n`/`Edit` contract, read-before-edit and
unchanged-file dedup as harness state, per-tool read-only/concurrency/
destructive metadata), and `Read`, `Edit`, `Write`, `Grep` and `Bash` were
rewritten against it. Its six settled positions are the middle rows of the
log above. Three findings were deliberately *not* adopted and are tracked
in `medium.md` §7 instead — a model-family fork of the edit format,
deferred tool loading behind a search tool, and a single-tool `Lsp` in
OpenCode's operation-enum shape.

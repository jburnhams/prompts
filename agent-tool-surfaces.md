...
The 23 sources in this collection with a dedicated "## Tool surface"
...
templates published in the editor's own repo, not a leak), plus the
four leaked closed-source extractions (Claude Code, Cursor, Windsurf,
...
of shared Windsurf-derived infrastructure post-acquisition). The
...
| Tier | Sources |
|---|---|
| No web/browser tool at all | OpenHands (`EXTERNAL_SERVICES` prefers API access over browsing), Aider, mini-swe-agent, Live-SWE-agent, Augment SWE-bench Agent (explicitly no internet access), Pi, Gemini CLI's captured snippet (no web-fetch constant imported here) |

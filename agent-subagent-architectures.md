...
| **A mailbox with path addressing — the same persistent-child shape one turn further, where notification and content are separate reads** | Codex CLI's v2 multi-agent toolset (read 2026-09-12; selected by `multi_agent_version: "v2"` in the model catalog, so the protocol shape is a per-model property). Four changes on its own v1 row above. **Agents are addressed by hierarchical path** — the root is told "You are `/root`", children see "`to=/root/...`, which indicates your identity is `/root/...`", and `list_agents` filters by "task-path prefix". **Two delivery verbs split on whether a turn happens**: `send_message` "will be delivered promptly. Does not trigger a new turn", while `followup_task` triggers one if the target is idle and otherwise lands "at message boundaries while sampling, or after the pending tool call completes" — nothing else in this table separates *inform* from *dispatch*. **`wait` returns a notification, not content**: "Does not return the content; returns either a summary of which agents have updates (if any), an interruption summary for steered input, or a timeout summary" — so the parent learns that something finished and then chooses what to read, where every other collector here pushes the child's output into the parent's context whether it was wanted or not. And the same `wait` "**also ends early when new user input is steered into the active turn**", so a parent blocked on children stays interruptible by the human without a second mechanism. Two smaller details: `fork_turns` replaces v1's boolean with `"none" \| "all" \| "3"`, making inherited history a number the parent picks; and "Completed agents remain open and count toward the concurrency limit until closed", which makes `close_agent` an obligation rather than a courtesy. |
...
| **Partial — a role-based config/persona layer confirmed to exist, but the actual prompt text per role wasn't read** | ~~Codex CLI — `agent/role.rs` defines named roles (`"default"`, `"explorer"`, `"worker"`)…~~ **Superseded 2026-09-12; moved to the row below.** The built-in role enum is gone: `agent-roles/` is now a crate that loads *user-defined* roles from TOML (`name`, `description`, `nickname_candidates`, plus a full config layer), so the per-role "prompt" is whatever a user wrote and there is no shipped set to capture. What *is* shipped is a delegation-position prompt, captured below. |
| **Yes — and it is a prompt for the agent's *position in the tree*, not for its task role, with both ends of the protocol written out** | Codex CLI at `multi_agent_version: "v2"` (read 2026-09-12; [`codex/gpt-6-astra_multi-agent-roles.md`](./codex/gpt-6-astra_multi-agent-roles.md)). The model catalog carries `multi_agent.role.root` and `multi_agent.role.subagent` — two short blocks, selected by where the agent sits rather than by what it was spawned to do. The root's opens "You are `/root`, the primary agent in a team of agents"; the child's opens "You are an agent in a team of agents collaborating to complete a task." Three things make this a different animal from every other "Yes" in this table, all of which describe a *specialisation*: (1) both blocks state the **same capability set** — "All agents in the team, including the agents that you can assign tasks to, are equally intelligent and capable, and have access to the same set of tools" — so the prompt establishes topology while explicitly denying hierarchy of ability; (2) they document the **inbound wire format** the agent will see (`Message Type: NEW_TASK \| MESSAGE \| FINAL_ANSWER`, `Task name`, `Sender`, `Payload`) rather than only what it may send, which no other captured sub-agent prompt does; and (3) both carry the same operational note that gives away who else is reading — "`send_message` calls may be read by a human, so ensure they are legible. Always put proper spaces between words and/or numbers." The child is additionally told that its `final` channel content "is immediately delivered back to your parent agent" **and** "may be read by a human", which is the clearest statement in this collection that inter-agent messages are a user-facing surface. Note the shape: a *user-defined* role (the TOML row above) supplies the specialisation, and the *catalog* supplies the protocol — the two layers this collection usually finds merged into one prompt. |
...
| **A hard numeric concurrency cap, not a qualitative rule** | ~~Codex CLI's `agent_jobs.rs` batch/CSV path caps concurrent spawned agents at 16 by default (max 64, configurable)~~ — **removed upstream on 2026-07-20** (`687f05c`, "Remove CSV-backed agent jobs"), leaving only dropped-table migration tests; the numbers no longer describe anything running. Kept struck through rather than deleted because the row was accurate when written and the *shape* — a numeric fan-out cap on a batch path, distinct from a file-conflict heuristic — is still the thing this row was naming. Codex's surviving numeric limit is the coordinator-enforced live-agent cap in the row below |
...
- **The layer above the call graph: nothing here except OpenClaw has
  ...
  **A second, weaker exception, added 2026-09-12: Codex's `/goal` is the
  layer above the *turn*, and it is instructive precisely because it stops
  short of being the layer above the run.** A thread goal is a persisted
  record with an objective, a token budget, usage accounting and a status
  (active / `complete` / `blocked` / `paused` / `budget_limited`); when a
  turn ends with the goal unfinished, the harness injects a continuation
  message and samples the model again. So an obligation outlives a *turn*
  by construction rather than by the model remembering to keep going —
  which is the half of the Workboard's job that concerns persistence of
  intent. What it does not do is outlive the thread or reach other agents:
  a goal is a property of one conversation, there is no dispatcher, no
  claiming, no dependency edges, and a child spawned under a goal does not
  inherit it. Read next to Workboard, the pair separates two things this
  document had been treating as one gap — *durable intent* (which `/goal`
  has and most sources lack) and *durable scheduling across agents* (which
  only Workboard has). A design can want the first without the second, and
  `/goal` is the cheapest existing demonstration of that, at the cost of one
  persisted record and one harness-side re-sample loop. Its prompt content
  is read in [`codex/README.md`](./codex/README.md); the parts worth
  stealing are the completion audit ("The audit must prove completion, not
  merely fail to find obvious remaining work") and the blocked audit's
  anti-gaming clause ("Treat equivalent blockers as the same condition
  across turns even when their wording or stated next step changes").

...
**Methodology note**: nine sources are covered. Five (Copilot Chat,
Goose, Crush, Pi, Gemini CLI) had their compaction prompt text already
sitting in this collection's stored files, just never written up as its
own section. Four (Claude Code, Codex CLI, OpenCode, OpenHands) required
going back to live upstream source — the same pattern established
across `agent-tool-surfaces.md` and `agent-subagent-architectures.md`,
where prompt-text-only extractions repeatedly undersold real
architecture. Every source's individual README has a "## Compaction"
section with full detail and file citations; this doc synthesizes across
them.

## Sources covered

Claude Code (leaked, via `leaked/claude-code/architecture-notes.md`),
Codex CLI, OpenCode, OpenHands, GitHub Copilot Chat, Goose, Crush, Pi,
Gemini CLI. Sources with **no compaction mechanism confirmed** in what's
captured — everything else in this collection. For most of those, that
means "not checked yet," not "confirmed absent" (see the Design
takeaways at the end) — the exceptions are sources whose architecture
...
- **The absence-of-evidence caveat from this collection's other
  synthesis docs applies here too, maybe more so**: four of nine
  sources (Goose, Crush, Pi, Gemini CLI) have a confirmed compaction
  *prompt* but zero confirmed trigger/threshold information, because
  the surrounding orchestration code was never part of what got
  ...
  these four is that real trigger logic exists and simply hasn't been
  looked at yet — not that it's absent.

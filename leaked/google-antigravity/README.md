...
cross-source comparison this feeds into. Antigravity's Knowledge Items
system is one of four independently-built **background distillation
agents** found in this collection (alongside Codex's Phase 1/Phase 2
pipeline, Gemini CLI's `confucius`, and Copilot CLI's `rem-agent`), and
the only one whose read-side prompt is written as an anti-redundant-work
mandate.

- **The writer is not model-invocable at all**: KIs are "generated by a
  separate KNOWLEDGE SUBAGENT that reads the conversations and then
  distills the information into new KIs or updates existing KIs as
  appropriate." The orchestrator can only read them. Nothing in the
  captures describes its prompt, trigger, or cadence.
- **Incremental at the corpus level**: "Individual KIs can be updated or
  expanded over multiple conversations" — anchor-and-update applied to a
  persistent, per-topic, cross-conversation knowledge base rather than to
  one session's summary.
- **Retrieval is mandatory and ordered.** "🚨 MANDATORY FIRST STEP: Check
  KI Summaries Before Any Research 🚨... BEFORE performing ANY research,
  analysis, or creating documentation, you MUST: Review the KI
  summaries... Identify relevant KIs... Read relevant KI artifacts...
  Build upon KI," with ❌/✅ worked examples and a two-mechanism
  precedence rule: "Search for relevant KIs first. Only read the
  conversation logs if there are no relevant KIs."
- **Retrospective use is called out explicitly** in the read-side
  triggers: "Before debugging unexpected behavior — Check if there are
  KIs documenting known bugs or gotchas"; "When config changes don't
  take effect — Check for KIs documenting configuration precedence";
  "When utility functions behave unexpectedly — Check for KIs about
  known bugs in common utilities."
- **A calibration caveat no other source states this bluntly**: "KIs are
  Starting Points, Not Ground Truth... KIs are snapshots from past work.
  They are valuable starting points, but **NOT** a substitute for
  independent research and verification. Always verify... Expect gaps...
  Question everything."
- **On-disk shape**: each KI under `<user>/.gemini/antigravity/knowledge`
  is `metadata.json` ("Summary, timestamps, and references to original
  sources") plus an `artifacts/` directory — summaries injected at
  conversation start, artifacts read by path on demand, raw
  `transcript.jsonl` reachable as the last resort. An index-plus-detail
  store with the raw record still queryable underneath it.
- **Absent from the CLI capture**: `CLI Prompt.md` references "KI
